...
bedrock.events.on('bedrock-mongodb.ready', function(callback) {
  ...
  async.auto({
    openCollections: function(callback) {
      database.openCollections(['role'], callback);
    },
    ...
    addRoles: ['createIndexes', function(callback) {
      async.eachSeries(bedrock.config.permission.roles, (role, callback) => {
        role.permanent = true;
 ...
 */
api.getPermissions = function(actor, callback) {
  async.auto({
    checkPermission: function(callback) {
      api.checkPermission(actor, PERMISSIONS.PERMISSION_ADMIN, callback);
    },
...
api.getRole = function(actor, id, callback) {
  if(!mongodbReady) {
    ...
    var role = bedrock.config.permission.roles[id];
    return callback(null, _.values(role));
  }
  async.waterfall([
    function(callback) {
    ...
    function(callback) {
      ...
      database.collections.role.findOne(q, function(err, result) {
        if(!result) {
...
api.getRoles = function(actor, callback) {
  ...
  async.waterfall([
    ...
    function(callback) {
      database.collections.role.find({}).toArray(function(err, results) {
        if(err) {
          return callback(err);
        }
...
api.addRole = function(actor, role, callback) {
  ...
  async.waterfall([
    ...
    function(callback) {
      database.collections.role.insert(
...
api.updateRole = function(actor, role, callback) {
  async.waterfall([
    ...
    function(callback) {
      database.collections.role.update(
...
api.removeRole = function(actor, id, callback) {
  async.waterfall([
    ...
    function(callback) {
      database.collections.role.deleteOne({id: database.hash(id)}, callback);
    }
  ], callback);
};
